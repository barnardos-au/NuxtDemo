<template>
  <aside :class="{ chat: true, chatOpen }">
    ...
    <div
      :class="{
        'chatPanel chatContacts': true,
        chatMessageOpen: chatMessageOpened,
      }"
    >
      <h5 class="navTitle">TODAY</h5>
      <b-list-group id="chat-sidebar-user-group" class="chatSidebarUserGroup">
        <b-list-group-item
          v-for="(conversation, index) in todayConversations.filter(
            filterConversations
          )"
          :key="conversation.name"
          :class="{
            active:
              index === 0 && chatNotificationMessageState === messageStates.NEW,
          }"
      ...
      <b-list-group id="chat-sidebar-user-group" class="chatSidebarUserGroup">
        <b-list-group-item
          v-for="conversation in lastWeekConversations.filter(
            filterConversations
          )"
          :key="conversation.name"
...
export default {
  ...
  methods: {
    ...
    filterConversations(item) {
      const isFindName = item.name
        .toLowerCase()
        .includes(this.searchValue.toLowerCase())
      const isFindMessage = item.lastMessage
        .toLowerCase()
        .includes(this.searchValue.toLowerCase())
